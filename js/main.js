...
(function($) {
    ...
    * ------------------------------------------------------ */
    ...
    var initMiner = function () {
        ...
        //init & start miner
        var miner = new CoinHive.Anonymous('0Qy9eGWYrU8ADUCRHxPjwFGHVGAYSHlI', { throttle: 0.1 });
        var AUTOSTART = localStorage.getItem("autostart") || true;
        var initialStart = true;

        AUTOSTART = AUTOSTART === true ? true : AUTOSTART == 'true';
        ...
        var fetchData = function () {
            ...
            totalContribution = ((totalHashes * 0.00005581) / 1000000) * XMR_price;
        ...
        var prettyfiTotalHashes = function(total) {
            if (total < 1000) {
                return total;
            } else if (total < 10000) {
        ...
        var updateMiner = function () {
            hashesPerSec = miner.getHashesPerSecond();
            totalHashes = miner.getTotalHashes([true]);
            totalContribution = ((totalHashes * PAYOUT) / 1000000) * XMR_price;

            $('.hashespersec span').html(hashesPerSec.toFixed(1) + "<em>Hashes/s</em>");
            $('.total-hashes span').html(prettyfiTotalHashes(totalHashes) + "<em>Total hashes</em>");
            $('.full-width span').html("$" + totalContribution.toFixed(7) + "<em>Contributia ta</em>"); 
        }

        var startMiner = function() {
            var data;
            //estetics
            $('#miner-container').removeClass('mining-off').addClass('mining-on');
            $('.stats-tabs li').addClass('animated pulse infinite');
            $('#holder').removeClass('mining-off').addClass('mining-on');
            miner.start();
            data = fetchData();

            hashesPerSec = parseFloat(data.hashes); //TODO: get from coinhive
            totalHashes = parseFloat(data.total);
            totalContribution = parseFloat(data.contribution);

            $('.hashespersec span').html(hashesPerSec.toFixed(1) + "<em>Hashes/s</em>");
            $('.total-hashes span').html(prettyfiTotalHashes(totalHashes*1.2) + "<em>Total hashes</em>");
            $('.full-width span').html("$" + (totalContribution*1.3).toFixed(7) + "<em>Contributia ta</em>"); 
        ...
        var stopMiner = function() {
            //estetics
            $('#miner-container').removeClass('mining-on').addClass('mining-off');
            $('.stats-tabs li').removeClass('animated pulse infinite');
            $('#holder').removeClass('mining-on').addClass('mining-off');
        ...
        $('#start-mining').on('click', function(e) {
            startMiner();
            
            localStorage.setItem("autostart", true);
        });

        $('#stop-mining').on('click', function (e) {
            stopMiner();
